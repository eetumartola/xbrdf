...
- 2026-07-14: Completed a renderer correctness/performance hardening pass. Fixed periodic shadow origins, explicit atlas sum reduction, shadowless dispatch sizing, and BVH stack-depth fallback; added checked scene/output allocations and scale-aware ray tolerances; batched atlas tiles; cached the process-wide GPU device, pipeline, BVH, and mesh buffers; and added adapter timestamp-query reporting. GPU contracts now cover periodic seam translation, uniform geometry scaling, progressive output, the 64/2048/65536 sample boundaries, atlas-versus-single output, and concurrent cache use. On the 129032-triangle fixture, the 32-light isotropic benchmark improved from 374.89 ms to 340.43 ms GPU dispatch (1.158 s to 1.073 s total). The 120000-sample single-light benchmark remained at 2.637 s GPU dispatch, with 2.633 s measured GPU compute time.
